...
export default {
    config: {
        ...
        keepFolding: {
            ...
            description: 'Everytime you click on one of the fold navigator element all code folds will be folded before the selected fold opens. Usefull if you want to keep your code folded all the time. Note that if you are using ctrl-alt-cmd-up/down keys to navigate it will not close the folds it had opened.',
            type: 'boolean',
            default: false
        },
        showLineNumbers: {
    ...
    observeTextEditors(editor) {
        ...
        /* OLD
        // fold all code on file open
        if (this.settings.autofold)
            editor.foldAll();

        editor.onDidStopChanging(() => this.parse(editor));
        */
    },
    ...
    observeActivePaneItem(pane) {
        ...
        */
        ...
        // if document changed subscirption
    ...
    parse(editor) {
        ...
        for (var i = 0; numberOfRows > i; i++) {
            if (editor.isFoldableAtBufferRow(i)) {
                ...
                html += '<div id="' + id + '" class="' + classList + '" data-row="' + i + '">' + gutter + lineNumberSpan + indentHtml + lineContent + '</div>';
            }
    ...
    navigatorElementOnClick() {
        ...
        element.onclick = (event) => {
            ...
            if (this.settings.keepFolding && !editor) {
                // fold all code before anything else
                editor.foldAll();
            }
