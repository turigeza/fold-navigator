...
            description: 'Everytime you click on one of the fold navigator elements all code folds will be folded before the selected fold opens. Useful if you want to keep your code folded all the time. Note that if you are using ctrl-alt-cmd-up/down keys to navigate it will not close the folds. Also you can temporarily enable/disable this behaviour by holding down the option key while you click. config.cson key : keepFolding',
            type: 'boolean',
            default: false
        },
        showLineNumbers: {
...
            description: 'Sometimes the fold falls on line which contains very little information. Typically comments like /** are meaningless. If the line content is less than this many characters use the next line for the fold description. config.cson key : minLineLength',
            type: 'integer',
            default: 6
        },
        keepFoldingAllTime: {
...
    },
    ...
    activate(state) {
        ...
        this.settings = null;
        */
        this.observeActivePaneItemDebouncedTimeout = null;
        this.settings = null;
        this.iniSettings();
        ...
        */
        ...
        atom.workspace.observeActivePaneItem(() => {
            this.observeActivePaneItemDebounced();
        });
        ...
        // });
        // atom.workspace.onDidChangeActiveTextEditor((item) => {
        ...
        //     console.log('observeActiveTextEditor');
        // });

        // atom.workspace.onDidAddPane((item) => {
        //     console.log('onDidAddPane');
        // });
        // atom.workspace.onWillDestroyPane((item) => {
        //     console.log('onWillDestroyPane');
        // });
        // atom.workspace.onDidDestroyPane((item) => {
        //     console.log('onDidDestroyPane');
        // });
        // atom.workspace.observePanes((item) => {
        //     console.log('observePanes');
        // });
        // atom.workspace.onDidChangeActivePane((item) => {
    ...
    observeActivePaneItemDebounced() {
        if(this.observeActivePaneItemDebouncedTimeout){
            clearTimeout(this.observeActivePaneItemDebouncedTimeout);
        }

        this.observeActivePaneItemDebouncedTimeout = setTimeout(() => {
